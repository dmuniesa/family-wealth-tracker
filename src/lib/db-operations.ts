...
export class UserService {
  ...
  static async getUsersByFamilyId(familyId: number): Promise<User[]> {
    const db = await getDatabase();
    return await db.all('SELECT * FROM users WHERE family_id = ?', [familyId]);
  }
...
export class AccountService {
  ...
  static async getAccountsByFamilyId(familyId: number): Promise<AccountWithBalance[]> {
    ...
    return accounts.map((account: AccountWithBalance) => ({
      ...account,
      iban_encrypted: decryptIBAN(account.iban_encrypted)
    }));
  }
  ...
  static async getDashboardData(familyId: number): Promise<{
    ...
    const currentTotals = await db.get(`
      SELECT 
        ...
        SUM(COALESCE(b.amount, 0)) as net_worth
      FROM accounts a
      LEFT JOIN (
        SELECT 
          account_id,
    ...
    const lastMonthTotals = await db.get(`
      SELECT 
        SUM(COALESCE(b.amount, 0)) as last_month_net_worth
      FROM accounts a
      LEFT JOIN (
        SELECT 
          account_id,
      ...
      WHERE a.family_id = ?
    `, [lastMonthStr, lastMonthStr, familyId]);
